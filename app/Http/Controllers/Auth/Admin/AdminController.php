...
public function addsoutenance()
    ...
    public function create_soutenance(Request $request)
    {
        ...
        {
            $data = Soutenance::with(['etudiant', 'enseignant', 'jury', 'sale'])->get();

            $layout = Auth::guard('admin')->check() ? 'auth.admindashboard' : 'auth.teacherdashboard';

            return view('auth.soutenancetable', compact('data', 'layout'));
        }
    ...
    public function updatesoutenance(Request $request, $id)
{
    ...
    public function storesalle(Request $request){
        ...
        $sale = new Sale([
            ...
            'depatement' => $request->input('department'), // <-- corriger ici aussi
        ]);
